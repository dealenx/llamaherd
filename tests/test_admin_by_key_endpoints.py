...
def test_get_by_key_endpoint():
    ...
    r = client.get("/admin/usage/by-key?token=test-token&days=30")
...
def test_get_by_key_costs_endpoint():
    _seed(proxy.usage_db)
    client = TestClient(proxy.app)
    r = client.get("/admin/usage/by-key-costs?token=test-token&days=30")
    assert r.status_code == 200
    data = r.json()
    assert "keys" in data
...
def test_get_by_key_detail_endpoint():
    _seed(proxy.usage_db)
    client = TestClient(proxy.app)
    r = client.get("/admin/usage/by-key/abcdef12?token=test-token&days=30")
    assert r.status_code == 200
    data = r.json()
...
def test_by_key_with_date_params():
    _seed(proxy.usage_db)
    client = TestClient(proxy.app)
    r = client.get("/admin/usage/by-key?token=test-token&start_date=2026-07-20&end_date=2026-07-20")
    assert r.status_code == 200
    data = r.json()
    assert len(data) == 1
...
def test_by_key_costs_with_key_filter():
    _seed(proxy.usage_db)
    client = TestClient(proxy.app)
    r = client.get("/admin/usage/by-key-costs?token=test-token&days=30&key=abcdef12")
    assert r.status_code == 200
    data = r.json()
    assert len(data["keys"]) == 1
...
def test_by_key_costs_with_key_filter_no_match():
    _seed(proxy.usage_db)
    client = TestClient(proxy.app)
    r = client.get("/admin/usage/by-key-costs?token=test-token&days=30&key=nonexistent")
    assert r.status_code == 200
    data = r.json()
    assert data["keys"] == []
